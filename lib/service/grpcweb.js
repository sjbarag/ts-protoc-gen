...
function printServicePromisesStub(methodPrinter, service) {
    var printer = new CodePrinter_1.CodePrinter(0, methodPrinter);
...
function printUnaryPromiseStubMethod(printer, method) {
    printer.printLn(method.serviceName + "PromisesClient.prototype." + method.nameAsCamelCase + " = function " + method.functionName + "(requestMessage) {")
        ...
        .dedent().printLn("} else {")
        .indent().printLn("resolve(responseMessage.toObject());")
        .dedent().printLn("}")
        .dedent().printLn("});")
        .dedent().printLn("});")
        .dedent().printLn("};");
...
function printPromiseServiceStubTypes(methodPrinter, service) {
    var printer = new CodePrinter_1.CodePrinter(0, methodPrinter);
...
function printUnaryPromiseStubMethodTypes(printer, method) {
